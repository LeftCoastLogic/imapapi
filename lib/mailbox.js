...
class Mailbox {
    ...
    async getStoredStatus() {
        ...
        data = data || {};
        if (!data.uidValidity) {
            const status = this.getMailboxStatus();
    ...
    async partialSync(storedStatus) {
        ...
        try {
            ...
            // console.log("DUYHO partialSync CONDSTORE messages", mailboxStatus.messages);
    ...
    async getMessageInfo(messageData, extended) {
        ...
        let { attachments, textId, encodedTextSize, plainTextParts, textPartFormats } = this.getAttachmentList(packedUid, messageData.bodyStructure);
    ...
    async listMessages(options) {
        ...
        try {
            ...
            for await (let messageData of this.imapClient.fetch(range, fields, opts)) {
                let messageInfo = await this.getMessageInfo(messageData, true);
                messages.push(messageInfo);
            }

            if (options.fetchPlain) {
                const groupedMessages =
                    _.groupBy(messages.filter(m => m.plainTextParts && m.plainTextParts.length),
                        m => m.plainTextParts[0]);
                await Promise.all(Object.keys(groupedMessages).map(async (part) => {
                    const uids = groupedMessages[part].map(m => m.uid);
                    for await (let messageData of this.imapClient.fetch(uids, {
                        uid: true,
                        bodyParts: [part]
                    }, { uid: true })) {
                        const message = groupedMessages[part].find(m => m.uid === messageData.uid);
                        let buffer = messageData.bodyParts.get(part);
                        let plainText = buffer ? decodeEmailBuffer(buffer, message.textPartFormats[part]) : '';
                        plainText = linkify(textToHtml(plainText), {
                            defaultProtocol: 'https'
                        });
                        message.text.plain = plainText;
                        delete message.plainTextParts;
                        delete message.textPartFormats;
                    }
                }));
            }
            return {
    ...
    async getMessagePreviews(messages) {
        let lock = await this.imapClient.getMailboxLock(this.path);
        clearTimeout(this.connection.completedTimer);
        try {
            const groupedMessages =
                _.groupBy(messages, m => m.textParts[0]);
            await Promise.all(Object.keys(groupedMessages).map(async (part) => {
                const uids = groupedMessages[part].map(m => m.uid);
                for await (let messageData of this.imapClient.fetch(uids, {
                    uid: true,
                    ...
                    const message = groupedMessages[part].find(m => m.uid === messageData.uid);
                    let buffer = messageData.bodyParts && messageData.bodyParts.get(part);
                    let text = '';
                    if (buffer && buffer.toString) {
                        text = libqp.decode(buffer.toString()).toString();
                        if (!isHtml(text)) {
                            text = linkify(textToHtml(text), {
                                defaultProtocol: 'https'
                            });
                        }
                    }
                    message.previewContent = text;
                }
            }));
        } catch (error) {
            this.logger.error(err);
...
function decodeEmailBuffer(buffer, format) {
    let decodedBuffer = '';
    format = format || {};
    const encoding = format.encoding ? format.encoding.toUpperCase() : '8BIT';
    let charset = format.charset || 'utf-8';
    if (charset) {
        charset = charset.toLowerCase();
        if (charset.substr(0, 4) === "win-") {
            charset = "windows-" + charset.substr(4);
        } else if (charset === "ks_c_5601-1987") {
            charset = "cp949";
        } else if (charset.match(/^utf\d/)) {
            charset = "utf-" + charset.substr(3);
        } else if (charset.match(/^latin[-_]?\d/)) {
            charset = "iso-8859-" + charset.replace(/\D/g, "");
        } else if (charset.match(/^(us-)?ascii$/)) {
            charset = "utf-8";
        } else if (charset.match(/^ansi_x3\.4-19/)) {
            // ANSI_X3.4-1968 and ANSI_X3.4-1986 are aliases for ASCII.
            // See http://en.wikipedia.org/wiki/ASCII#Aliases
            charset = "utf-8";
        }
    }
    if ((/base64/gi).test(encoding)) {
        decodedBuffer = Buffer.from(buffer.toString(), 'base64');
    } else if ((/quot(ed)?[-/]print(ed|able)?/gi).test(encoding)) {
        // if (charset === 'utf-8') {
        //   decodedBuffer = Buffer.from(qp.decode(utf8.decode(buffer)));
        // } else {
        //   decodedBuffer = Buffer.from(qp.decode(buffer));
        // }
        decodedBuffer = mimelib.decodeQuotedPrintable(buffer, charset);
    } else if (encoding === '7BIT') {
        decodedBuffer = buffer.toString('ascii');
    } else if (encoding === '8BIT' || encoding === 'BINARY') {
        // decodedBuffer = iconvlite.decode(Buffer.from(buffer), 'win1258');
        decodedBuffer = Encoding.convert(buffer, 'utf-8', charset).toString('utf-8');
    }

    if (encoding === '7BIT') decodedBuffer = decodedBuffer.toString('ascii');
    else decodedBuffer = decodedBuffer.toString();
